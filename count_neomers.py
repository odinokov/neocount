#!/usr/bin/env python3
"""count_neomers v0.2 — neomer detection from BAM/CRAM."""
...
from ndb import AF_BIN_LABELS, NmerDB, decode_kmer, process_pair_batch_dense
...
def _validate_args(ap, args) -> None:
    ...
    if args.threads > 4:
        print(
            "WARNING: --threads is for BAM/CRAM BGZF decompression; capping to 4", file=sys.stderr
        )
        args.threads = 4
...
class _PairBatch:
    """Accumulates mate pairs and flushes them through the dense Numba counter.

    Owns the batch arrays and grows their per-read width on demand. A flush is
    triggered automatically when the batch fills or a longer read arrives, and
    on flush the epoch counter rolls over before it can overflow uint32.
    """
    ...
    def __init__(
        ...
        self._read_width = read_width
        self._group_counts = group_counts
        self._observed_entries = observed_entries
        self._seen_group = seen_group
        self._counted_group = counted_group
        self._emit_neomers = emit_neomers
        self._seq1, self._seq2, self._len1, self._len2 = _alloc_pair_batch(batch_size, read_width)
        self._n = 0
        self._epoch = 1

    def add(self, seq1: str, seq2: str) -> None:
        needed_width = max(len(seq1), len(seq2))
        if needed_width > self._read_width or self._n == self._batch_size:
            self.flush()
            if needed_width > self._read_width:
                self._read_width = _next_batch_width(self._read_width, needed_width)
                self._seq1, self._seq2, self._len1, self._len2 = _alloc_pair_batch(
                    self._batch_size, self._read_width
                )
        self._len1[self._n] = _store_seq(self._seq1[self._n], seq1)
        self._len2[self._n] = _store_seq(self._seq2[self._n], seq2)
        self._n += 1

    def flush(self) -> None:
        if self._n == 0:
            return
        if self._epoch + self._n >= _MAX_EPOCH:
            self._seen_group.fill(0)
            self._counted_group.fill(0)
            self._epoch = 1
        self._epoch = process_pair_batch_dense(
            self._seq1,
            self._len1,
            self._seq2,
            self._len2,
...
def _log_progress(n_reads: int, n_pairs: int, reads_last: int, t_last: float, log_every: int):
    ...
    print(
        f"[{n_reads / 1e6:.1f}M reads, {n_pairs / 1e6:.1f}M pairs, {speed / 1e6:.2f}M reads/s]",
        file=sys.stderr,
        flush=True,
    )
    return now, n_reads
...
def _scan_reads(bam_path: str, db: NmerDB, args):
    ...
    norm_dict = _DEFAULT_NORM_REGIONS
    ...
    batch = _PairBatch(
        db,
        args.batch_size,
        args.batch_read_width,
        group_counts,
        observed_entries,
        np.zeros(n_groups, dtype=np.uint32),
        np.zeros(n_groups, dtype=np.uint32),
        args.emit_neomers,
    )
    ...
    with pysam.AlignmentFile(bam_path, "rb", **open_kwargs) as bam:
        for read in bam:
            ...
            if seq1 is None:
                if _tlen_ok(tlen, len(seq), db.k):
                    n_evicted += _enqueue_read(pending, qname, seq, args.max_pending)
                else:
                    n_tlen_filtered += 1
            elif _tlen_ok(tlen, min(len(seq), len(seq1)), db.k):
                batch.add(seq1, seq)
                n_pairs += 1
            else:
                n_tlen_filtered += 1
    ...
    stats = dict(
        ...
        n_pairs=n_pairs,
        n_evicted=n_evicted,
        n_tlen_filtered=n_tlen_filtered,
        n_norm=n_norm,
        elapsed=elapsed,
    )
    ...
    for gid in np.flatnonzero(group_counts):
        ...
        writer.writerow(
            [
                ...
                g["variant_class"],
                af_bin,
                AF_BIN_LABELS[af_bin],
                int(group_counts[gid]),
                norm_label,
                norm_reads,
                neomer_str,
            ]
...
def main():
    ...
    ap.add_argument(
        "--batch-size", type=int, default=100_000, help="Read pairs per Numba dense-lookup batch"
    )
    ap.add_argument(
        "--batch-read-width",
        type=int,
    ...
    if n["n_pairs"] == 0 and n["n_reads"] > 1000:
        print(
            "WARNING: 0 pairs formed; input may be single-end or unsorted — "
            "requires paired-end reads for neomer counting",
            file=sys.stderr,
        )
    if n["n_reads"] > 0 and n["n_evicted"] / n["n_reads"] > 0.01:
        print(
            f"WARNING: {pct_evict:.1f}% of filtered reads evicted from pending cache; "
            "increase --max-pending",
            file=sys.stderr,
        )
